...
class ADTab(tk.Frame):
    ...
    # dong
    def displat_search_button(self):
        search_text = self.search_input.get()
        print("test search input:"+search_text)
    
    # dong
    def select_group(self):
        print("test select group")
    
    # dong
    def select_name(self):
        print("test select name")
    ...
    def create_page(self):
        ...
        self.search_input = tk.Entry(self.frame)
        self.search_input.insert(0, "Search input box")
        self.search_input.grid(row=1, column=2, columnspan=1)

        self.search_button = tk.Button(self.frame,text="Search",command=self.displat_search_button)
        self.search_button.grid(row=1, column=3, columnspan=1)

        self.display_all_mc = tk.Button(self.frame,text='Display all Machines')
        self.display_all_mc.grid(row=0,column=5,columnspan=1)

        #checkbox button for selecting all machines listed
        self.AllMachines = tk.BooleanVar()
        
        self.display_by_group = ttk.Checkbutton(self.frame, text="Select by Group", onvalue=True, offvalue=False, command=self.select_group)
        self.display_by_group.grid(row=1, column=5, columnspan=1)
        self.display_by_name = ttk.Checkbutton(self.frame, text="Select by Name", onvalue=True, offvalue=False, command=self.select_name)
        self.display_by_name.grid(row=2, column=5, columnspan=1)
